...
const getStatus = (state: Types.RequestLogic.STATE, expectedAmount: BigNumber, balance: BigNumber | undefined, pending: boolean) => {
...
export const parseRequest = async ({
  ...
  const canceledTimestamp = data.events.find((x) => x.name === Types.RequestLogic.ACTION_NAME.CANCEL)?.timestamp

  const extensionsValues = Object.values(data.extensions).find((x) => x.type === 'payment-network')?.values
  ...
  return {
    ...
    canceledDate: canceledTimestamp ? new Date(canceledTimestamp * 1000) : undefined,
